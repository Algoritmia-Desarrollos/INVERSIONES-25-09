...
// (El resto del archivo 'dashboard.js' sigue exactamente igual)
...
function renderSummary(accounts, portfolioItems) {
    ...
    const totalCash = accounts.filter(acc => acc.currency !== 'PORTFOLIO').reduce((sum, acc) => sum + acc.initial_balance, 0);
    const netWorth = totalPortfolioValue + totalCash;
    summaryContainer.innerHTML = `<div class="summary-card"><h3>Patrimonio Neto</h3><p>${formatCurrency(netWorth)}</p></div><div class="summary-card"><h3>Inversiones</h3><p>${formatCurrency(totalPortfolioValue)}</p></div><div class="summary-card"><h3>Efectivo y Bancos</h3><p>${formatCurrency(totalCash)}</p></div>`;
}
function renderAccounts(accounts, portfolioItems) {
    ...
    accountsList.innerHTML = accounts.map(acc => {
        const balance = acc.currency === 'PORTFOLIO' ? totalPortfolioValue : acc.initial_balance;
        return `<div class="account-card"><span class="name">${acc.name}</span><span class="balance">${formatCurrency(balance, acc.currency === 'PORTFOLIO' ? 'ARS' : acc.currency)}</span><span class="currency">${acc.currency}</span></div>`;
    }).join('');
}
function renderTransactions(transactions) {
    ...
    transactionsList.innerHTML = `<table class="min-w-full"><tbody>${transactions.map(tr => `<tr class="border-b border-gray-200"><td class="py-3 px-4">${tr.description}</td><td class="py-3 px-4 text-gray-500">${tr.account_name}</td><td class="py-3 px-4 text-right font-medium ${tr.amount >= 0 ? 'text-green-600' : 'text-red-600'}">${formatCurrency(tr.amount)}</td></tr>`).join('')}</tbody></table>`;
}
